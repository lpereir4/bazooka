...
const (
	...
	CryptKeyPath      = "/bazooka-cryptokey"
	MetaFolder        = "/meta"
...
func main() {
	...
	for _, file := range files {
		...
		if err != nil {
			log.Fatal(fmt.Errorf("Error while generating a dockerfile: %v", err))
		}
	}
...
func resolveLanguageParser(language lib.BzkString) (string, error) {
	...
	if val, ok := parserMap[string(language)]; ok {
		return val, nil
	}
